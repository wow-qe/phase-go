...
// PriorEvidenceSummary is what a phase actually left on the record — a LIVE
// scan, not a cached status: a phase that returned cleanly while recording
// a failing result reads Passed in its outcome row, and a guard deciding on
// that lie would gate wrongly at exactly the moment gating matters.
